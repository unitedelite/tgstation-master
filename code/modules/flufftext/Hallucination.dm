...
/datum/hallucination/chat/New(mob/living/carbon/C, forced = TRUE, force_radio, specific_message)
	...
	var/speak_messages = list("[pick_list_replacements(HAL_LINES_FILE, "suspicion")]",\
		...
		"Je suis infecté, [pick_list_replacements(HAL_LINES_FILE, "infection_advice")]!")

	var/radio_messages = list("[pick_list_replacements(HAL_LINES_FILE, "people")] est [pick_list_replacements(HAL_LINES_FILE, "accusations")]!",\
		"A l'aide!",\
		"[pick_list_replacements(HAL_LINES_FILE, "threat")] dans [pick_list_replacements(HAL_LINES_FILE, "location")][prob(50)?"!":"!!"]",\
		"[pick("Où est [target.first_name()]?", "Mettez [target.first_name()] en arrest!")]",\
		"[pick("IA, appele","Appelez","Recall")] la shuttle!",\
		"IA [pick("rogue", "morte")]!!")
