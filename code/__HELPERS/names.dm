			...
			if(2)
				switch(rand(1,3))//Food, drinks, or places. Only selectable once.
					...
					if(2)
						word_en = pick(foods)
						. += lowertext(word_en)
						. += lowertext(strings_translate(ION_FILE, "ionfood", word_en))
					if(3)
			...
			if(3)
				switch(rand(1,4))//Abstract nouns, objects, adjectives, threats. Can be selected more than once.
					if(1)
						word_en = pick(nouns)
						. += lowertext(word_en)
						. += lowertext(strings_translate(ION_FILE, "ionabstract", word_en))
					if(2)
						word_en = pick(objects)
						. += lowertext(word_en)
						. += lowertext(strings_translate(ION_FILE, "ionobjects", word_en))
					if(3)
						word_en = pick(adjectives)
						. += lowertext(word_en)
						. += lowertext(strings_translate(ION_FILE, "ionadjectives", word_en))
					if(4)
						word_en = pick(threats)
						. += lowertext(word_en)
						. += lowertext(strings_translate(ION_FILE, "ionthreats", word_en))
		if(!return_list)
			if(words==1)
				. += "."
			else
